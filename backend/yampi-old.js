...
function getColunasItensDisponiveis() {
  ...
  try {
    ...
    console.log(`✅ Detectadas ${_colunasItensCache.size} colunas na tabela itens_pedido_yampi`);
    return _colunasItensCache;
  } catch (error) {
...
function montarInsertPedido(dados) {
  ...
  Object.keys(mapeamento).forEach(coluna => {
    if (colunasDisponiveis.has(coluna)) {
      colunas.push(coluna);
      valores.push(mapeamento[coluna]);
      placeholders.push('?');
    }
  });
...
function montarUpdatePedido(dados) {
  ...
  valores.push(dados.yampi_order_id);
...
async function salvarItemPedido(pedidoId, item) {
  console.log(`🔧 DEBUG salvarItemPedido - pedidoId: ${pedidoId}, item.id: ${item.id}`);
  
  const colunasDisponiveis = getColunasItensDisponiveis();
  console.log('🔧 DEBUG colunas disponíveis:', Array.from(colunasDisponiveis));
  ...
  console.log('🔧 DEBUG dadosItem:', JSON.stringify(dadosItem, null, 2));
  ...
  Object.keys(dadosItem).forEach(coluna => {
    if (colunasDisponiveis.has(coluna)) {
      colunas.push(coluna);
      valores.push(dadosItem[coluna]);
      placeholders.push('?');
    } else {
      console.log(`⚠️  Coluna ${coluna} não existe na tabela`);
    }
  });
